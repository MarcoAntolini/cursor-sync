...
> **Setup note**: This skill ships only with its `SKILL.md`. The companion `scripts/search.py` and `data/` CSV files were external dependencies in the source repo (under `src/ui-ux-pro-max/`) that are not bundled here. The Quick Reference rules and Checklists in this file are self-contained and usable without the scripts. To enable the `python3 search.py` CLI commands shown below, drop the `scripts/` and `data/` folders from the upstream repo into this skill directory.
