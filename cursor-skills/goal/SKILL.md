---
name: goal
description: Define concrete, measurable goals before work begins, including success criteria, evidence, scope, and stop conditions. Use when explicitly invoked as /goal, goal, goal skill, or when the user asks to create a goal, set an objective, clarify success criteria, or turn a fuzzy intention into a verifiable outcome.
...
This skill covers goal definition and refinement only. Do not create planning ledgers, durable snapshots, decision logs, or long-running execution artifacts from this skill.

## Workflow

1. Confirm that goal definition is needed.
   - Use this skill only when the user explicitly invokes it or asks to define a goal.
   - If the user asks for ordinary implementation work, do the work directly instead of forcing goal creation.
...
5. Check active goal state if goal tools are available.
   - If tools such as `get_goal` and `create_goal` are available, call `get_goal` before creating a new goal.
   - If no active goal exists and the objective meets the quality bar, create the goal.
   - If an active goal still matches the user's intent, continue using it instead of creating a duplicate.
   - If an active goal conflicts with the new request, ask whether to finish the current goal, mark it complete if done, or start a separate goal-backed thread.
   - If no goal tools are available in Cursor, present the goal statement in chat and ask for confirmation before using it.

6. Create or present the goal only after it passes the quality bar.
   - Use a single concise objective string.
   - Include verification evidence in the objective itself.
   - Include scope bounds when they constrain the work.
   - Include a token budget only when the user explicitly requested one.

## Goal Quality Bar

Before setting or presenting a goal, the objective should answer:
...
Weak: "Make checkout faster." or "Keep investigating the PR comments."
