...
python3 ".cursor/skills/swiftui-ios-expert/scripts/record_trace.py" --list-templates
```
...
```bash
TRACE=$(python3 ".cursor/skills/swiftui-ios-expert/scripts/record_trace.py" \
    --attach Helm --stop-file /tmp/stop-trace --output ~/Desktop/session.trace \
    2>&1 | awk '/trace written:/ {print $NF}')
python3 ".cursor/skills/swiftui-ios-expert/scripts/analyze_trace.py" --trace "$TRACE" --json-only
```
