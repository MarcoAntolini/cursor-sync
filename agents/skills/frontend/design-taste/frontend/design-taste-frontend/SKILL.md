...
description: Anti-slop frontend skill for marketing landing pages, portfolios, and promotional redesigns only — not dashboards, data tables, or multi-step product UI (use impeccable for those). Reads the brief, infers design direction, ships non-templated interfaces. Real design systems when applicable, audit-first on redesigns, strict pre-flight check.
---
